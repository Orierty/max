...
def handle_accept_request(
    ...
    needy_user = get_user(needy_user_id)
    needy_username = needy_user.get("username", "неизвестно")

    print("needy_user_id", needy_user_id)

    print("needy_user", needy_user)

    print("request", request)

    # Формируем ссылку на чат
    # needy_link = f"https://web.max.ru/{user_id}"
    needy_link = f"https://web.max.ru/8115399"
    ...
    send_message_with_keyboard(
        volunteer_chat_id,
        (
            f"✅ Вы приняли запрос от пользователя @{needy_username}!{stats_text}\n\n"
            f"💬 Чтобы перейти в чат, откройте ссылку:\n{needy_link}\n\n"
            f"После завершения диалога нажмите кнопку ниже."
        ),
        buttons,
    )

    # --- Уведомляем нуждающегося ---
    volunteer_link = f"https://web.max.ru/{user_id}"

    text = (
        f"✅ Волонтёр @{volunteer_username or 'неизвестно'} принял ваш запрос и готов помочь!\n\n"
        f"💬 Для связи откройте чат: {volunteer_link}"
    )
    send_message(needy_user_id, text)
